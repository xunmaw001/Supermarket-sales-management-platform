        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '会员编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.huiyuanUuidNumber" placeholder="会员编号" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '会员姓名' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.huiyuanName" placeholder="会员姓名" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('huiyuan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chaoshishouhuoguanlipingtai/upload/huiyuanMuBan.xls"
                        >批量导入会员数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('huiyuan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="huiyuanUploadSuccess"
                                :on-error="huiyuanUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('huiyuan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "huiyuan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="huiyuanUuidNumber"
                                   header-align="center"
                                   label="会员编号">
                        <template slot-scope="scope">
                            {{scope.row.huiyuanUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="会员手机号">
                        <template slot-scope="scope">
                            {{scope.row.huiyuanPhone}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="huiyuanIdNumber"
                                   header-align="center"
                                   label="会员身份证号">
                        <template slot-scope="scope">
                            {{scope.row.huiyuanIdNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="huiyuanPhoto"
                                     header-align="center"
                                     width="200"
                                     label="会员头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.huiyuanPhoto">
                                <img :src="$base.url+scope.row.huiyuanPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('huiyuan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('huiyuan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('huiyuan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                huiyuanUuidNumber : null,
                huiyuanName : null,
                huiyuanPhone : null,
                huiyuanIdNumber : null,
                huiyuanPhoto : null,
                sexTypes : null,
                newMoney : null,
                huiyuanEmail : null,
            ...
            json_fields: {
                //级联表字段
                //本表字段
                     '会员编号': "huiyuanUuidNumber",
                     '会员姓名': "huiyuanName",
                     '会员手机号': "huiyuanPhone",
                     '会员身份证号': "huiyuanIdNumber",
                     '会员头像': "huiyuanPhoto",
                     '性别': "sexValue",
                     '余额': "newMoney",
                     '会员邮箱': "huiyuanEmail",
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'huiyuan',//当前表表名,
                        sumColum : 'huiyuan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'huiyuan_name',//分组字符串字段
                        // types : 'huiyuan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.huiyuanUuidNumber!= '' && this.searchForm.huiyuanUuidNumber!= undefined) {
                    params['huiyuanUuidNumber'] = '%' + this.searchForm.huiyuanUuidNumber + '%'
                }
         
                if (this.searchForm.huiyuanName!= '' && this.searchForm.huiyuanName!= undefined) {
                    params['huiyuanName'] = '%' + this.searchForm.huiyuanName + '%'
                }
                                                        
                params['huiyuanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            huiyuanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "huiyuan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
