    ...
    <div class="addEdit-block">
        <el-form
            ...
            <el-row>
                ...
                <el-col :span="12"  v-if="sessionTable !='shangpinChuruInout'">
                    <el-form-item class="select" v-if="type!='info'"  label="出入库" prop="shangpinChuruInoutId">
                        <el-select v-model="ruleForm.shangpinChuruInoutId" :disabled="ro.shangpinChuruInoutId" filterable placeholder="请选择出入库" @change="shangpinChuruInoutChange">
                            <el-option
                                    v-for="(item,index) in shangpinChuruInoutOptions"
                                    v-bind:key="item.id"
                                    :label="item.shangpinChuruInoutName"
                                    :value="item.id">
                            </el-option>
                        </el-select>
                    </el-form-item>
                ...
                <el-col :span="12"  v-if="sessionTable !='shangpinChuruInout' ">
                    <el-form-item class="input" v-if="type!='info'"  label="出入库流水号" prop="shangpinChuruInoutUuidNumber">
                        <el-input v-model="shangpinChuruInoutForm.shangpinChuruInoutUuidNumber"
                                  placeholder="出入库流水号" clearable readonly></el-input>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="出入库流水号" prop="shangpinChuruInoutUuidNumber">
                            <el-input v-model="ruleForm.shangpinChuruInoutUuidNumber"
                                      placeholder="出入库流水号" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12"  v-if="sessionTable !='shangpinChuruInout' ">
                    <el-form-item class="input" v-if="type!='info'"  label="出入库名称" prop="shangpinChuruInoutName">
                        <el-input v-model="shangpinChuruInoutForm.shangpinChuruInoutName"
                                  placeholder="出入库名称" clearable readonly></el-input>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="出入库名称" prop="shangpinChuruInoutName">
                            <el-input v-model="ruleForm.shangpinChuruInoutName"
                                      placeholder="出入库名称" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12"  v-if="sessionTable !='shangpinChuruInout' ">
                    <el-form-item class="input" v-if="type!='info'"  label="出入库类型" prop="shangpinChuruInoutValue">
                        <el-input v-model="shangpinChuruInoutForm.shangpinChuruInoutValue"
                                  placeholder="出入库类型" clearable readonly></el-input>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="出入库类型" prop="shangpinChuruInoutValue">
                            <el-input v-model="ruleForm.shangpinChuruInoutValue"
                                      placeholder="出入库类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <input id="updateId" name="id" type="hidden">
            <input id="shangpinChuruInoutId" name="shangpinChuruInoutId" type="hidden">
            <input id="shangpinId" name="shangpinId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="操作数量" prop="shangpinChuruInoutListNumber">
                       <el-input v-model="ruleForm.shangpinChuruInoutListNumber"
                                 placeholder="操作数量" clearable  :readonly="ro.shangpinChuruInoutListNumber"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="操作数量" prop="shangpinChuruInoutListNumber">
                           <el-input v-model="ruleForm.shangpinChuruInoutListNumber"
...
<script>
    ...
    export default {
        ...
        created() {
         ...
         this.$http({
             url: `shangpinChuruInout/page?page=1&limit=100`,
             method: "get"
         }).then(({ data }) => {
             if (data && data.code === 0) {
                this.shangpinChuruInoutOptions = data.data.list;
            }
         });
        ...
        methods: {
            ...
            shangpinChuruInoutChange(id){
                this.$http({
                    url: `shangpinChuruInout/info/`+id,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.shangpinChuruInoutForm = data.data;
                    }
                });
            },
            // 多级联动参数
            info(id) {
                let _this =this;
                _this.$http({
                    url: `shangpinChuruInoutList/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        _this.ruleForm = data.data;
                        _this.shangpinChange(data.data.shangpinId)
                        _this.shangpinChuruInoutChange(data.data.shangpinChuruInoutId)
                    } else {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`shangpinChuruInoutList/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.shangpinChuruInoutListCrossAddOrUpdateFlag = false;
                                        this.parent.search();
