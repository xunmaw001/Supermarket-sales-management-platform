	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.gongyingshangId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('gongyingshang', this.gongyingshangId);
                    this.detail = res.data
					this.detail.gongyingshangContent = this.detail.gongyingshangContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.gongyingshangPhoto ? this.detail.gongyingshangPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('gongyingshangYuyue', {
