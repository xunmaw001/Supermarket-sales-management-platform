	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0px 0px 0px rgba(0, 0, 0, 0.16)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"0 180rpx 180rpx 0","borderWidth":"4rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}'
						v-model="searchForm.gongyingshangName" type="text" placeholder="供应商名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('gongyingshang','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('gongyingshang','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.gongyingshangName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('gongyingshang','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.gongyingshangUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">供应商唯一编号</text>
                     <text style="width: 70%">
                         <text v-if="item.gongyingshangUuidNumber != null">{{item.gongyingshangUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.lianxirenPhone" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">联系人手机号</text>
                     <text style="width: 70%">
                         <text v-if="item.lianxirenPhone != null">{{item.lianxirenPhone}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.lianxirenAddress" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">地址</text>
                     <text style="width: 70%">
                         <text v-if="item.lianxirenAddress != null">{{item.lianxirenAddress}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.gongyingshangValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">供应商类型</text>
                     <text style="width: 70%">{{item.gongyingshangValue}}</text>
                 </view>
                 <view v-show="item.gongyingshangXinyongdengjiValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">信用等级</text>
                     <text style="width: 70%">{{item.gongyingshangXinyongdengjiValue}}</text>
                 </view>
                <view v-show="item.gongyingshangContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">供应商详情</text>
                    <view style="width: 70%" v-html="item.gongyingshangContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('gongyingshang','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('gongyingshang','审核') && item.gongyingshangYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`gongyingshang`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('gongyingshang', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`gongyingshang`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.gongyingshangName) {
                    searchForm['gongyingshangName'] = this.searchForm.gongyingshangName
                }
				if (this.searchForm.lianxirenName) {
                    searchForm['lianxirenName'] = this.searchForm.lianxirenName
                }

                let res = await this.$api.page(`gongyingshang`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.gongyingshangContent!=null){
                        item.gongyingshangContent = item.gongyingshangContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
