	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0px 0px 0px rgba(0, 0, 0, 0.16)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"0 180rpx 180rpx 0","borderWidth":"4rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}'
						v-model="searchForm.shangpinChuruInoutName" type="text" placeholder="出入库名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('shangpinChuruInout','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('shangpinChuruInout','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='shangpinChuruInoutTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in shangpinChuruInoutTypesList" :key="index" :class="shangpinChuruInoutTypes===item.codeIndex?'tabActive':''"
                                  @tap="shangpinChuruInoutTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.shangpinChuruInoutName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('shangpinChuruInout','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.shangpinChuruInoutUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">出入库流水号</text>
                     <text style="width: 70%">
                         <text v-if="item.shangpinChuruInoutUuidNumber != null">{{item.shangpinChuruInoutUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.shangpinChuruInoutValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">出入库类型</text>
                     <text style="width: 70%">{{item.shangpinChuruInoutValue}}</text>
                 </view>
                <view v-show="item.shangpinChuruInoutContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">备注</text>
                    <view style="width: 70%" v-html="item.shangpinChuruInoutContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('shangpinChuruInout','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('shangpinChuruInout','审核') && item.shangpinChuruInoutYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
                ...
                this.shangpinChuruInoutTypesSelect()
		...
		methods: {
            ...
            //当前表的 出入库类型 字段 字典表查询方法
            async shangpinChuruInoutTypesSelect(){
                let shangpinChuruInoutTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'shangpin_churu_inout_types',
                });
                shangpinChuruInoutTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.shangpinChuruInoutTypesList = shangpinChuruInoutTypes.data.list
            },
            //当前表的 出入库类型字段类别搜索
			shangpinChuruInoutTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.shangpinChuruInoutTypes != 0) {
                    params.shangpinChuruInoutTypes = this.shangpinChuruInoutTypes
                }
                let _this = this
                let res = await this.$api.page(`shangpinChuruInout`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('shangpinChuruInout', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`shangpinChuruInout`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.shangpinChuruInoutName) {
                    searchForm['shangpinChuruInoutName'] = this.searchForm.shangpinChuruInoutName
                }

                let res = await this.$api.page(`shangpinChuruInout`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.shangpinChuruInoutContent!=null){
                        item.shangpinChuruInoutContent = item.shangpinChuruInoutContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
