	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0px 0px 0px rgba(0, 0, 0, 0.16)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"0 180rpx 180rpx 0","borderWidth":"4rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}'
						v-model="searchForm.huiyuanName" type="text" placeholder="会员姓名"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('huiyuan','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('huiyuan','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.huiyuanName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('huiyuan','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.huiyuanUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">会员编号</text>
                     <text style="width: 70%">
                         <text v-if="item.huiyuanUuidNumber != null">{{item.huiyuanUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.huiyuanPhone" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">会员手机号</text>
                     <text style="width: 70%">
                         <text v-if="item.huiyuanPhone != null">{{item.huiyuanPhone}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.huiyuanIdNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">会员身份证号</text>
                     <text style="width: 70%">
                         <text v-if="item.huiyuanIdNumber != null">{{item.huiyuanIdNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 ...
                 <view v-show="item.newMoney" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">余额</text>
                     <text style="width: 70%">
                         <text v-if="item.newMoney != null">{{item.newMoney}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.huiyuanEmail" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">会员邮箱</text>
                     <text style="width: 70%">
                         <text v-if="item.huiyuanEmail != null">{{item.huiyuanEmail}}</text>
                         <text v-else></text>
                     </text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('huiyuan','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('huiyuan','审核') && item.huiyuanYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.list(`huiyuan`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('huiyuan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`huiyuan`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.huiyuanUuidNumber) {
                    searchForm['huiyuanUuidNumber'] = this.searchForm.huiyuanUuidNumber
                }
				if (this.searchForm.huiyuanName) {
                    searchForm['huiyuanName'] = this.searchForm.huiyuanName
                }

                let res = await this.$api.list(`huiyuan`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
