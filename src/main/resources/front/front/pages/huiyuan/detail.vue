	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('huiyuan', this.huiyuanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.huiyuanPhoto ? this.detail.huiyuanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('huiyuanYuyue', {
