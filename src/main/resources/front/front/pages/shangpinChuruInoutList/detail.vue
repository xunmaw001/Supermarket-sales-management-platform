	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.shangpinChuruInoutListId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('shangpinChuruInoutList', this.shangpinChuruInoutListId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.shangpinChuruInoutListPhoto ? this.detail.shangpinChuruInoutListPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('shangpinChuruInoutListYuyue', {
