	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.shangpinOrderId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('shangpinOrder', this.shangpinOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.shangpinOrderPhoto ? this.detail.shangpinOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('shangpinOrderYuyue', {
									yonghuId: _this.user.id,
									shangpinOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/shangpinOrderYuyue/list');
							}
						}
					});
				},
